...
$('.btn_new_cliente').click(function(e) {
  ...
  $('#pais_cliente_cliente').removeAttr('disabled');
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response == 0) {
        ...
        $('#calle_cliente').val(data.calle);
        $('#municipio_cliente').val(data.municipio);
        $('#pais_cliente_cliente').val(data.pais_cliente);
        $('#estado').val(data.estado);
        $('#cp_cliente').val(data.CP);
        ...
        $('#calle_cliente').attr('disabled','disabled');
        $('#municipio_cliente').attr('disabled','disabled');
        $('#pais_cliente_cliente').attr('disabled','disabled');
        $('#estado').attr('disabled','disabled');
        $('#cp_cliente').attr('disabled','disabled');
...
$('#txt_codproducto').keyup(function(e) {
  e.preventDefault();
...
$('#add_product_venta').click(function(e) {
  ...
  if ($('#txt_cant_producto').val() > 0) {
    var codproducto = $('#txt_codproducto').val();
...
// facturar venta
$('#btn_hacer_venta').click(function(e) {
  e.preventDefault();
  var rows = $('#detalle_venta tr').length;
  if (rows > 0) {
      // Verificar si la tarjeta de crédito está lista
      var card = elements.getElement('card');
      stripe.createToken(card).then(function(result) {
          if (result.error) {
              // Mostrar errores en caso de problemas con la tarjeta
              console.error(result.error.message);
              alert('Error en la tarjeta de crédito: ' + result.error.message);
          } else {
              // Si no hay errores, continuar con el procesamiento de venta
              var action = 'procesarVenta';
              var codcliente = $('#idcliente').val();
              var token = result.token.id;

              // Envía el token de la tarjeta al servidor junto con otros datos necesarios
              $.ajax({
                  url: 'modal.php',
                  type: 'POST',
                  async: true,
                  data: { action: action, codcliente: codcliente, stripeToken: token },
                  success: function(response) {
                      if (response != 0) {
                          var info = JSON.parse(response);
                          // Genera el PDF y recarga la página
                          generarPDF(info.codcliente, info.nofactura);
                          location.reload();
                      } else {
                          console.log('No hay dato');
