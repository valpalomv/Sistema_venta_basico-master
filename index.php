...
if (!empty($_SESSION['active'])) {
    ...
    if (!empty($_POST)) {
        if (empty($_POST['usuario']) || empty($_POST['clave']) || empty($_POST['tipo_usuario'])) {
            $alert = '<div class="alert alert-danger" role="alert">Ingrese su usuario, su clave y seleccione el tipo de usuario</div>';
        } else {
            ...
            $query = mysqli_query($conexion, "SELECT u.idusuario, u.nombre, u.correo, u.usuario, r.idrol, r.rol FROM usuario u INNER JOIN rol r ON u.rol = r.idrol WHERE u.usuario = '$user' AND u.clave = '$clave' AND r.rol = '$tipoUsuario'");
            ...
            if ($resultado > 0) {
                ...
                $alert = '<div class="alert alert-danger" role="alert">Usuario, Contraseña o Tipo de Usuario Incorrectos</div>';
                session_destroy();
            }
        }
    }
